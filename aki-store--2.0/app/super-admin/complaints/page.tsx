...
import { MessageSquareWarning, ArrowRight, X, ShieldAlert } from "lucide-react";

const mockComplaints = [
    { id: "CPL-9012", store: "Urban Edge", customer: "james@example.com", issue: "Non-delivery of goods", severity: "High", status: "Open", date: "Oct 24, 2026" },
    { id: "CPL-9013", store: "Aura Home", customer: "claire@email.com", issue: "Counterfeit claim", severity: "Critical", status: "Investigating", date: "Oct 23, 2026" },
    { id: "CPL-9014", store: "Minimalist Drop", customer: "mark@buy.net", issue: "Rude customer service", severity: "Low", status: "Resolved", date: "Oct 20, 2026" },
];
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                            {mockComplaints.filter(c => c.status === activeTab).map((complaint) => (
                                <tr key={complaint.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                    <td className="p-6 font-cinzel text-xs tracking-wider text-gray-900 dark:text-white">{complaint.id}</td>
                                    <td className="p-6">
                                        <p className="font-cinzel font-semibold tracking-wider text-gray-900 dark:text-white uppercase">{complaint.store}</p>
                                        <p className="text-[10px] text-gray-500 tracking-widest">{complaint.customer}</p>
                                    </td>
                                    <td className="p-6 text-sm text-gray-600 dark:text-gray-400">{complaint.issue}</td>
                                    <td className="p-6">
                            ...
                            {mockComplaints.filter(c => c.status === activeTab).length === 0 && (
                                <tr>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
                    <div className="w-full max-w-lg bg-[#fcfcfc] dark:bg-[#050505] border border-gray-200 dark:border-white/10 p-8 relative">
                        ...
                        <h2 className="font-cinzel text-2xl tracking-wider text-gray-900 dark:text-white uppercase mb-6 flex items-center gap-3">
                            <ShieldAlert className="h-6 w-6 text-red-600 dark:text-red-500" /> Case {selectedComplaint.id}
                        </h2>
                        ...
                        <div className="space-y-4 mb-8 text-sm font-light tracking-wide text-gray-600 dark:text-gray-400">
                            <div className="flex justify-between border-b border-gray-200 dark:border-white/10 pb-4">
                                ...
                                <span className="text-gray-900 dark:text-white">{selectedComplaint.store}</span>
                            </div>
                            <div className="flex justify-between border-b border-gray-200 dark:border-white/10 pb-4">
                                <span className="uppercase text-[10px] font-semibold tracking-widest text-gray-500">Customer</span>
                                <span className="text-gray-900 dark:text-white">{selectedComplaint.customer}</span>
                            </div>
                            <div className="flex justify-between border-b border-gray-200 dark:border-white/10 pb-4">
                                <span className="uppercase text-[10px] font-semibold tracking-widest text-gray-500">Issue</span>
                                <span className="text-gray-900 dark:text-white font-medium">{selectedComplaint.issue}</span>
                            </div>
                            ...
                            <div className="pt-2">
                                ...
                                <div className="p-4 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 italic text-sm">
                                    "I placed an order two weeks ago and the vendor keeps ignoring my messages. I want a refund."
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4">
                            ...
                            {selectedComplaint.status !== 'Resolved' && (
                                <button onClick={() => setIsModalOpen(false)} className="flex-1 border border-red-900 bg-red-900 py-3 text-xs font-semibold uppercase tracking-widest text-white transition-colors hover:bg-transparent hover:text-red-900 dark:hover:text-red-500">
                                    Escalate to Vendor
                                </button>
                            )}
                        </div>
